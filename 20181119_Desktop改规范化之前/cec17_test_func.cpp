...
void cec17_test_func(double *x, double *f, int nx, int mx, int func_num)
{
	...
	if (ini_flag == 0)
	{
		...
		if (fpt == NULL)
		{
			printf("\n Error: Cannot open input file for reading MMMMMM\n");
		}
		if (func_num<20)
		{
		...
		if (fpt == NULL)
		{
			printf("\n Error: Cannot open input file for reading OOOOOOOSSS\n");
		}
		...
		if (func_num >= 11 && func_num <= 20)
		{
			...
			if (fpt == NULL)
			{
				printf("\n Error: Cannot open input file for reading shuffle_data_2\n");
			}
		...
		else if (func_num == 29 || func_num == 30)
		{
			...
			if (fpt == NULL)
			{
				printf("\n Error: Cannot open input file for reading shuffle_data_\n");
			}
	...
	for (i = 0; i < mx; i++)
	{
		...
		*/
		case 6:
			sphere_func(&x[i*nx], &f[i], nx, OShift, M, 0, 0);
			f[i] += 600.0;
			break;
		case 7:
